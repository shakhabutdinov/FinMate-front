...
import { SubscriptionStatus, Transaction, Goal, Asset, StockHolding, CryptoHolding } from '../../models/api.models';
  ...
  template: `
    <div class="flex items-center justify-between px-4 py-3 bg-gray-900/80 backdrop-blur-md border-b border-gray-800 sticky top-0 z-20">
      ...
      <div class="flex items-center gap-2">
        ...
        <button (click)="openSearch()"
          ...
          <span class="hidden lg:inline text-[10px] px-1.5 py-0.5 rounded border" style="border-color: var(--border-1);">⌘K</span>
        </button>
        <button (click)="openSearch()" class="sm:hidden p-2 text-gray-400 hover:text-white rounded-full hover:bg-white/5 transition-colors" aria-label="Search">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
        </button>

        <!-- Theme toggle -->
        <button (click)="theme.toggle()" [attr.aria-label]="theme.theme() === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'"
          ...
          <span class="absolute top-0.5 h-7 w-7 rounded-full transition-all duration-300 flex items-center justify-center"
            [class]="theme.theme() === 'dark'
            ...
            @if (theme.theme() === 'dark') {
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"/></svg>
            } @else {
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="4"/><path d="M12 2v2"/><path d="M12 20v2"/><path d="m4.93 4.93 1.41 1.41"/><path d="m17.66 17.66 1.41 1.41"/><path d="M2 12h2"/><path d="M20 12h2"/><path d="m4.93 19.07 1.41-1.41"/><path d="m17.66 6.34 1.41-1.41"/></svg>
            }
          </span>
          <span class="absolute left-2 top-1/2 -translate-y-1/2 text-[10px] font-medium opacity-60"
            [class.opacity-0]="theme.theme() === 'dark'"
            [style.color]="'var(--text-4)'">DARK</span>
          <span class="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] font-medium opacity-60"
            [class.opacity-0]="theme.theme() === 'light'"
            [style.color]="'var(--text-4)'">LIGHT</span>
        </button>

        <!-- Notifications trigger -->
        <button (click)="openNotifications()"
          class="p-2 text-gray-400 hover:text-white rounded-full hover:bg-white/5 transition-colors relative" aria-label="Notifications">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.268 21a2 2 0 0 0 3.464 0"/><path d="M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326"/></svg>
          @if (unreadCount() > 0) {
            <span class="absolute top-0.5 right-0.5 min-w-[18px] h-[18px] px-1 rounded-full bg-[#00FF88] text-black text-[10px] font-bold flex items-center justify-center">
    ...
    @if (showSearch) {
      <div class="fixed inset-0 z-50" (click)="closeSearch()">
        ...
        <div class="relative mx-auto mt-16 lg:mt-20 w-full max-w-[640px] px-4 animate-fade-in" (click)="$event.stopPropagation()">
          <div class="rounded-2xl border shadow-2xl overflow-hidden"
            style="background: var(--surface-modal); border-color: var(--border-1);">
            <div class="flex items-center gap-3 px-4 h-14 border-b" style="border-color: var(--border-1);">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: var(--text-3);"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
              <input #searchInput
                [(ngModel)]="searchQuery"
              ...
              @if (searchQuery) {
                <button (click)="searchQuery = ''" class="p-1 rounded hover:bg-black/5">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: var(--text-3);"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
                </button>
              }
              <kbd class="hidden sm:inline-flex text-[10px] px-2 py-0.5 rounded border" style="border-color: var(--border-1); color: var(--text-4);">Esc</kbd>
            </div>
            ...
            <div class="max-h-[60vh] overflow-y-auto">
              @if (searchLoading) {
                ...
                <div class="px-4 py-3">
                  <div class="text-[11px] font-semibold uppercase tracking-wider mb-2" style="color: var(--text-4);">Quick navigation</div>
                  <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    @for (q of quickLinks; track q.href) {
                      <button (click)="go(q.href)" class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors hover:bg-[#00FF88]/10"
                ...
                <div class="px-6 py-10 text-center">
                  ...
                  <div class="text-sm" style="color: var(--text-3);">No matches for "{{ searchQuery }}"</div>
                </div>
              } @else {
                @for (group of groupedResults(); track group.name) {
                  <div class="px-4 pt-3 pb-1 text-[11px] font-semibold uppercase tracking-wider" style="color: var(--text-4);">{{ group.name }}</div>
                  @for (r of group.items; track r.title + r.subtitle) {
                    <button (click)="go(r.href)"
                      ...
                      <div class="flex-1 min-w-0">
                        <div class="text-sm truncate" style="color: var(--text-1);">{{ r.title }}</div>
                        <div class="text-xs truncate" style="color: var(--text-3);">{{ r.subtitle }}</div>
                      </div>
                      @if (r.amount !== undefined) {
                        <div class="text-sm font-medium whitespace-nowrap" style="color: var(--text-2);">{{ r.amount | currency:'USD':'symbol':'1.0-2' }}</div>
                      }
                    </button>
                  }
                }
    ...
    @if (showNotifications) {
      <div class="fixed inset-0 z-50" (click)="showNotifications = false">
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm lg:bg-transparent lg:backdrop-blur-0"></div>
        <div class="absolute top-0 right-0 w-full max-w-[430px] mx-auto left-0 lg:left-auto lg:right-2 lg:w-[380px]" (click)="$event.stopPropagation()">
          <div class="m-3 mt-14 rounded-2xl shadow-2xl overflow-hidden border animate-fade-in"
            style="background: var(--surface-modal); border-color: var(--border-1);">
            <div class="flex items-center justify-between px-4 py-3 border-b" style="border-color: var(--border-1);">
              <div class="flex items-center gap-2">
                <span class="font-semibold" style="color: var(--text-1);">Notifications</span>
                @if (unreadCount() > 0) {
                  <span class="text-[10px] font-bold px-2 py-0.5 rounded-full" style="background: rgba(0,255,136,0.18); color: #00FF88;">{{ unreadCount() }} new</span>
                }
              </div>
              <button (click)="markAllRead()" class="text-xs hover:underline" style="color: var(--text-3);">Mark all read</button>
            </div>
            ...
            <div class="max-h-[70vh] overflow-y-auto">
              @if (notifications().length === 0) {
                <div class="px-6 py-10 text-center">
                  <div class="text-3xl mb-2">✨</div>
                  <div class="text-sm" style="color: var(--text-3);">You're all caught up!</div>
                </div>
              } @else {
                @for (n of notifications(); track n.id) {
                  <button (click)="onNotificationClick(n)"
                    class="w-full flex items-start gap-3 px-4 py-3 text-left transition-colors hover:bg-[#00FF88]/5"
                    style="border-bottom: 1px solid var(--border-2);">
                    <div class="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0"
                      [style.background]="kindBg(n.kind)" [style.color]="kindFg(n.kind)">
                      @switch (n.kind) {
                        @case ('success') { <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg> }
                        @case ('warning') { <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 9v4"/><path d="M12 17h.01"/><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/></svg> }
                        @default  { <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg> }
                      }
                    </div>
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2">
                        <span class="text-sm font-medium" style="color: var(--text-1);">{{ n.title }}</span>
                        @if (!readIds().has(n.id)) {
                          <span class="w-2 h-2 rounded-full bg-[#00FF88]"></span>
                        }
                      </div>
                      <div class="text-xs mt-0.5" style="color: var(--text-3);">{{ n.body }}</div>
                      <div class="text-[10px] mt-1 uppercase tracking-wide" style="color: var(--text-4);">{{ n.when }}</div>
                    </div>
                  </button>
                }
              }
    ...
    @if (showAccountPanel) {
      <div class="fixed inset-0 z-50" (click)="showAccountPanel = false">
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm lg:bg-transparent lg:backdrop-blur-0"></div>
        <div class="absolute top-0 right-0 w-full max-w-[430px] mx-auto left-0 lg:left-auto lg:right-2 lg:w-[360px]" (click)="$event.stopPropagation()">
...
export class TopNavbarComponent implements OnInit {
  ...
  searchQuery = '';
  searchLoading = false;
  private searchData = signal<{ transactions: Transaction[]; assets: Asset[]; goals: Goal[]; stocks: StockHolding[]; crypto: CryptoHolding[] }>({
    transactions: [], assets: [], goals: [], stocks: [], crypto: []
  });
  ...
  filteredResults = computed<SearchResult[]>(() => {
    ...
    for (const a of d.assets) {
      if ((a.name + ' ' + a.type).toLowerCase().includes(q)) {
        out.push({ group: 'Asset', title: a.name, subtitle: a.type, amount: a.balance, href: this.routeForAsset(a.type) });
      }
    }
    for (const g of d.goals) {
      if (g.name.toLowerCase().includes(q)) {
        out.push({ group: 'Goal', title: g.name, subtitle: `${g.progressPercent}% complete`, amount: g.targetAmount, href: '/pfm' });
      }
    }
    for (const s of d.stocks) {
      if ((s.symbol + ' ' + s.companyName).toLowerCase().includes(q)) {
        out.push({ group: 'Stock', title: `${s.symbol} · ${s.companyName}`, subtitle: `${s.quantity} shares`, amount: s.totalValue, href: '/stocks' });
      }
    }
    for (const c of d.crypto) {
      if ((c.symbol + ' ' + c.name).toLowerCase().includes(q)) {
        out.push({ group: 'Crypto', title: `${c.symbol} · ${c.name}`, subtitle: `${c.amount} units`, amount: c.totalValue, href: '/crypto' });
      }
    }
  ...
  constructor(public authService: AuthService, private api: ApiService, public theme: ThemeService, private router: Router) {}
  ...
  resultIcon(g: SearchResult['group']) {
    return g === 'Transaction' ? '💸' : g === 'Asset' ? '💼' : g === 'Goal' ? '🎯' : g === 'Stock' ? '📈' : '₿';
  }
  resultBg(g: SearchResult['group']) {
    return g === 'Transaction' ? 'rgba(248,113,113,0.15)' :
           g === 'Asset' ? 'rgba(0,255,136,0.15)' :
           g === 'Goal' ? 'rgba(168,85,247,0.15)' :
           g === 'Stock' ? 'rgba(59,130,246,0.15)' :
                           'rgba(245,158,11,0.15)';
  }
  resultFg(g: SearchResult['group']) {
    return g === 'Transaction' ? '#f87171' :
           g === 'Asset' ? '#00FF88' :
           g === 'Goal' ? '#a855f7' :
           g === 'Stock' ? '#3b82f6' :
                           '#f59e0b';
  ...
  kindBg(k: NotificationItem['kind']) {
    return k === 'success' ? 'rgba(0,255,136,0.15)' : k === 'warning' ? 'rgba(245,158,11,0.15)' : 'rgba(59,130,246,0.15)';
  }
  kindFg(k: NotificationItem['kind']) {
    return k === 'success' ? '#00FF88' : k === 'warning' ? '#f59e0b' : '#3b82f6';
  }
  ...
  activateWithGooglePay() {
    ...
    try {
      ...
      const paymentDataRequest = {
        ...
        merchantInfo: { merchantId: 'BCR2DN4T7OL6JMGF', merchantName: 'FinMate' },
        transactionInfo: { totalPriceStatus: 'FINAL', totalPrice: '9.99', currencyCode: 'USD', countryCode: 'US' }
      };
      paymentsClient.loadPaymentData(paymentDataRequest)
        .then((paymentData: any) => this.processPayment(paymentData.paymentMethodData.tokenizationData.token))
        .catch(() => { this.activating = false; this.payError = 'Payment was cancelled.'; });
    } catch {
      this.processPayment('gpay_simulated_token');
    }
  }
  ...
  private loadSearchAndNotifications() {
    ...
    const safe = <T>(obs: any, fb: T) => (obs as any).pipe(catchError(() => of(fb)));
    forkJoin({
  ...
  private rebuildNotifications() {
    ...
    for (const t of recent) {
      list.push({
        ...
        title: t.type === 'Income' ? `Income · ${t.category}` : `Spent on ${t.category}`,
  ...
  private routeForAsset(type: string): string {
    return type === 'Stock' ? '/stocks' : type === 'Crypto' ? '/crypto' : '/dashboard';
  }
  ...
  private persistReadIds(s: Set<string>) {
    try { localStorage.setItem('finmate_read_notifications', JSON.stringify(Array.from(s))); } catch {}
  }
}
