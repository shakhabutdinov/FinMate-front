        ...
        @if (loading) {
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-4 pb-20">
            <div class="flex flex-col items-center justify-center pt-8 pb-4">
              <div
                class="w-20 h-20 rounded-2xl bg-[#00FF88]/10 flex items-center justify-center mb-4"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="size-10 text-[#00FF88]"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="1.5"
              ...
              <p class="text-gray-400 text-sm text-center px-6 mb-6">
                Connect your Alpaca brokerage account to trade stocks and view
                your positions in real time.
              </p>
            </div>
              ...
              >
                ...
                <div class="flex items-center gap-2 pt-1">
                  ...
                  >
                    <input
                      type="checkbox"
                      formControlName="isPaper"
                      class="sr-only peer"
                    />
                    <div
                      class="w-9 h-5 bg-gray-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-[#00FF88]"
                    ></div>
                  </label>
                  <span class="text-gray-400 text-xs"
                    >Paper Trading Account</span
                  >
                </div>
                @if (error) {
            ...
            <div
              class="bg-gray-800/20 border border-gray-700/50 rounded-2xl p-4"
            >
              <div class="flex items-start gap-3">
                ...
                <div class="text-xs text-gray-400 leading-relaxed">
                  ...
                  <ol class="list-decimal list-inside space-y-0.5">
                    <li>
                      Go to
                      <span class="text-[#00FF88]">alpaca.markets</span> and
                      sign in
                    </li>
                    <li>Navigate to Paper Trading &gt; API Keys</li>
                    <li>
                      Click <span class="text-[#00FF88]">Generate</span> to
          ...
          <div class="flex-1 overflow-y-auto pb-20">
            <div class="p-4 space-y-5">
              <div class="text-center space-y-1 py-4">
                <div class="flex items-center justify-center gap-3">
                  ...
                  @if (alpacaData.isPaper) {
                    <span
                      class="text-[10px] font-medium bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full"
                      >PAPER</span
                    >
                  }
                  <button
                ...
                <div class="text-4xl font-bold text-white">
                  {{ alpacaData.equity | currency: 'USD' : 'symbol' : '1.2-2' }}
                </div>
                <div
                  class="flex items-center justify-center gap-4 text-xs text-gray-400 pt-1"
                >
                  <span
                    >Cash:
                    <span class="text-white font-medium">{{
                      alpacaData.cash | currency: 'USD' : 'symbol' : '1.2-2'
                    }}</span></span
                  >
                  <span
                    >Buying Power:
                    <span class="text-white font-medium">{{
                      alpacaData.buyingPower
              ...
              @if (showLogoutConfirm) {
                <div
                  class="bg-gray-900/80 border border-red-500/30 rounded-2xl p-4"
                >
                  <p class="text-white text-sm font-medium mb-3 text-center">
                    Disconnect Alpaca account?
                  </p>
                  <div class="flex gap-3">
                    <button
                      (click)="showLogoutConfirm = false"
                      class="flex-1 py-2 px-3 bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm rounded-xl transition-colors font-medium"
                    >
                      No
                    </button>
                    <button
                      (click)="disconnect()"
                      class="flex-1 py-2 px-3 bg-red-500/20 hover:bg-red-500/30 text-red-400 text-sm rounded-xl transition-colors font-medium border border-red-500/30"
                    >
                      Yes, Disconnect
                    </button>
                  </div>
              ...
              @if (alpacaData.positions.length > 0) {
                    ...
                    >
                      <div class="flex items-center gap-3">
                        ...
                        <div class="text-left">
                          ...
                          <div class="text-xs text-gray-400">
                            {{ pos.qty }} shares @
                            {{ pos.avgEntryPrice | currency: 'USD' }}
                          </div>
                        </div>
                      </div>
                      <div class="text-right">
                        ...
                        >
                          ...
                          ({{ pos.unrealizedPlPc >= 0 ? '+' : ''
                          }}{{ pos.unrealizedPlPc * 100 | number: '1.2-2' }}%)
                        </div>
                      </div>
                    </button>
                  }
                ...
                <div class="text-center py-8">
                  <p class="text-gray-500 text-sm">
                    No open positions. Place a buy order below to get started.
                  </p>
                </div>
              }
                ...
                <div class="space-y-3">
                  <div class="flex items-center justify-between px-1">
                    <h3 class="text-gray-400 text-sm font-medium">
                      {{ selectedPosition.symbol }} Price (30d)
                    </h3>
                    @if (chartLoading) {
                      <div
              ...
              >
                ...
                <form [formGroup]="tradeForm" class="space-y-4">
                  ...
                  <div
                    class="flex rounded-xl overflow-hidden border border-gray-700"
                  >
                    <button
                      type="button"
                      (click)="setSide('buy')"
                ...
                @if (tradeError) {
                  <div
                    class="bg-red-500/10 border border-red-500/30 rounded-xl p-3 text-red-400 text-xs"
                  >
                    {{ tradeError }}
                  </div>
                }
                ...
                @if (lastOrderResult) {
                  <div
                    class="bg-[#00FF88]/10 border border-[#00FF88]/30 rounded-xl p-3 space-y-1"
                  >
                    <p class="text-[#00FF88] text-xs font-semibold">
                      Order submitted successfully
                    </p>
                    <p class="text-gray-300 text-xs">
                      {{ lastOrderResult.side | titlecase }}
                      {{ lastOrderResult.qty }} × {{ lastOrderResult.symbol }} —
                      {{ lastOrderResult.type | titlecase }}
                    </p>
                    <p class="text-gray-400 text-xs">
                      Status:
                      <span class="text-white">{{
                        lastOrderResult.status
                      }}</span>
                    </p>
                    @if (lastOrderResult.filledAvgPrice) {
                      <p class="text-gray-400 text-xs">
                        Filled @
                        <span class="text-white">{{
                          lastOrderResult.filledAvgPrice | currency: 'USD'
                        }}</span>
                      </p>
                    }
                  </div>
                ...
                <button
                  type="button"
                  (click)="placeOrder()"
                  [disabled]="placingOrder || tradeForm.invalid"
              ...
              @if (orders.length > 0) {
                <div class="space-y-3">
                  <div class="flex items-center justify-between px-1">
                    ...
                    <button
                      (click)="loadOrders()"
                      class="text-[#00FF88] text-xs hover:underline"
                    >
                      Refresh
                    </button>
                  </div>
                  @for (order of orders; track order.orderId) {
                    <div
                      class="flex items-center justify-between p-3 bg-gray-900/50 rounded-xl border border-gray-800"
                    >
                      <div class="flex items-center gap-3">
                        <div
                      ...
                      <div class="text-right">
                        ...
                        @if (order.filledAvgPrice) {
                          <div class="text-gray-400 text-xs mt-0.5">
                            @ {{ order.filledAvgPrice | currency: 'USD' }}
                          </div>
                        }
                        @if (
                          order.status === 'new' ||
                          ...
                          <button
                            ...
                            class="text-red-400 text-xs hover:underline mt-1 block"
                          >
                            Cancel
                          </button>
                        }
...
export class StocksComponent implements OnInit {
  ...
  onConnect() {
    ...
    this.api.connectAlpaca(apiKey!, secretKey!, isPaper ?? true).subscribe({
      ...
      error: (err) => {
        this.error =
          err.error?.error || 'Failed to connect. Check your API keys.';
        this.connecting = false;
      },
    });
  }
  ...
  placeOrder() {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (err) => {
          this.tradeError =
            err.error?.error || 'Order failed. Please try again.';
          this.placingOrder = false;
        },
      });
  }
