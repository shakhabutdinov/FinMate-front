...
import { CryptoPortfolio, CryptoHolding, TrendingCrypto } from '../../models/api.models';
            ...
            <div class="p-4 space-y-5">
              ...
              <div class="text-center space-y-1 py-4">
                ...
                <div class="text-4xl font-bold text-white">
                  {{ portfolio?.totalBalance | currency:'USD':'symbol':'1.2-2' }}
                </div>
                <div class="flex items-center justify-center gap-2 pt-1">
                  <span class="text-xs font-medium px-2 py-0.5 rounded-full"
                    [class]="(portfolio?.changePercent ?? 0) >= 0
                      ? 'bg-[#00FF88]/15 text-[#00FF88]'
                      : 'bg-red-500/15 text-red-400'">
                    {{ (portfolio?.changePercent ?? 0) >= 0 ? '+' : '' }}{{ portfolio?.changePercent | number:'1.2-2' }}% today
                  </span>
                  <span class="text-xs text-gray-500">
                    {{ (portfolio?.changeAmount ?? 0) >= 0 ? '+' : '' }}{{ portfolio?.changeAmount | currency:'USD':'symbol':'1.2-2' }}
                  </span>
                </div>
                <p class="text-gray-600 text-xs pt-1">Live prices via Binance public feed</p>
              </div>

              <!-- ── HOLDINGS ────────────────────────────────────────── -->
              @if (portfolio && portfolio.holdings.length > 0) {
                <div class="space-y-3">
                  <h3 class="text-gray-400 text-sm font-medium px-1">Your Holdings</h3>
                  @for (holding of portfolio.holdings; track holding.id) {
                    <button
                      ...
                      [class]="selectedHolding?.id === holding.id
                      ...
                      <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm"
                             [style.background-color]="getAssetBgColor(holding.symbol)"
                             [style.color]="getAssetTextColor(holding.symbol)">
                          {{ holding.symbol.charAt(0) }}
                        </div>
                        <div class="text-left">
                          <div class="font-medium">{{ holding.symbol }}</div>
                          <div class="text-xs text-gray-400">{{ holding.amount }} coins @ {{ holding.pricePerUnit | currency:'USD':'symbol':'1.2-4' }}</div>
                        </div>
                      </div>
                      <div class="flex items-center gap-3">
                        <div class="text-right">
                          <div class="font-medium">{{ holding.totalValue | currency:'USD':'symbol':'1.2-2' }}</div>
                        </div>
                        <button type="button"
                          (click)="$event.stopPropagation(); deleteHolding(holding.id)"
                ...
                <div class="text-center py-6">
                  <p class="text-gray-500 text-sm">No holdings yet. Add your first crypto below.</p>
                </div>
              }
              ...
              @if (selectedHolding) {
                <div class="space-y-3">
                  <div class="flex items-center justify-between px-1">
                    <h3 class="text-gray-400 text-sm font-medium">{{ selectedHolding.symbol }}/USDT — 30 Day Price</h3>
                    @if (chartLoading) {
              ...
              <div class="bg-gray-800/40 border border-gray-700 rounded-2xl p-5 space-y-4">
                ...
                @if (showForm) {
                  <form [formGroup]="addForm" (ngSubmit)="onAdd()" class="space-y-3">

                    <!-- Crypto selector / quick picks -->
                    <div>
                      <label class="text-gray-400 text-xs mb-2 block">Quick Select</label>
                      <div class="flex flex-wrap gap-2">
                        @for (coin of popularCoins; track coin) {
                          <button type="button" (click)="quickSelect(coin)"
                            class="px-3 py-1 rounded-lg text-xs font-medium border transition-colors"
                            [class]="addForm.get('symbol')?.value === coin
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="text" formControlName="symbol" placeholder="BTC" (blur)="fetchLivePrice(addForm.get('symbol')?.value || '')"
                      ...
                      <div>
                        <label class="text-gray-400 text-xs mb-1 block">Name</label>
                        <input type="text" formControlName="name" placeholder="Bitcoin"
                    ...
                    @if (fetchingPrice) {
                      <div class="flex items-center gap-2 text-gray-400 text-xs">
                        <div class="w-3 h-3 border border-[#00FF88] border-t-transparent rounded-full animate-spin"></div>
                        Fetching live price...
                      </div>
                    }
                    @if (livePrice && !fetchingPrice) {
                      <div class="bg-[#00FF88]/10 border border-[#00FF88]/30 rounded-xl px-4 py-3 flex justify-between items-center">
                        <span class="text-gray-400 text-xs">Live Price (Binance)</span>
                        <span class="text-[#00FF88] font-semibold text-sm">\${{ livePrice | number:'1.2-4' }}</span>
                      </div>
                    }
                    @if (livePriceError) {
                      <div class="text-red-400 text-xs">{{ livePriceError }}</div>
                    }

                    <div>
                      <label class="text-gray-400 text-xs mb-1 block">Amount (coins)</label>
                      <input type="number" formControlName="amount" placeholder="0.5" min="0" step="any"
                        class="w-full h-11 bg-gray-800/50 border border-gray-700 text-white text-sm placeholder:text-gray-500 rounded-lg px-3 focus:outline-none focus:ring-2 focus:ring-[#00FF88]/50 transition-all">
                    </div>
                    ...
                    <button type="submit" [disabled]="addForm.invalid || adding || !livePrice"
                ...
                <div class="space-y-3">
                  ...
                  <div class="grid grid-cols-2 gap-3">
                    @for (item of portfolio.trending; track item.symbol) {
                      <button type="button" (click)="quickSelect(item.symbol)"
                        ...
                        <div class="mt-2 text-xs text-gray-400">{{ item.price | currency:'USD':'symbol':'1.2-2' }}</div>
                      </button>
                    }
                  </div>
                </div>
...
export class CryptoComponent implements OnInit {
  ...
  readonly popularCoins = ['BTC', 'ETH', 'SOL', 'BNB', 'XRP', 'ADA', 'DOGE', 'AVAX'];
  fetchingPrice = false;
  ...
  private readonly coinNames: Record<string, string> = {
    ...
    PEPE: 'Pepe', TIA: 'Celestia', ARB: 'Arbitrum', SEI: 'Sei'
  };
  ...
  lineChartOptions: ChartConfiguration<'line'>['options'] = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        bodyColor: '#fff', borderColor: '#374151', borderWidth: 1,
        padding: 10, displayColors: false,
        callbacks: {
          label: (ctx) => `$${Number(ctx.raw).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 })}`
        }
      }
    },
    scales: {
  ...
  loadPortfolio() {
    this.api.getCryptoPortfolio().subscribe({
      next: (data) => {
        this.portfolio = data;
        this.loading = false;
        // Auto-select first holding for chart
  ...
  onAdd() {
    ...
    // Price is fetched server-side from Binance — send 0 as fallback
    const pricePerUnit = this.livePrice ?? 0;

    this.api.createCryptoHolding({
      symbol:      symbol.toUpperCase(),
      name,
      amount,
      pricePerUnit,
      color:       this.assetColors[symbol.toUpperCase()]?.bg ?? 'rgba(107,114,128,0.125)'
    }).subscribe({
      next: () => {
        this.adding = false;
        this.addForm.reset();
